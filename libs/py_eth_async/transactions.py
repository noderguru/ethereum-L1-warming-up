...
class Tx(AutoRepr):
    ...
    async def cancel(
        ...
        if self.params and 'nonce' in self.params:
            if not gas_price:
                gas_price = (await client.transactions.gas_price()).Wei
    ...
    async def speed_up(
        ...
        if self.params and 'nonce' in self.params:
            if not gas_price:
                gas_price = int((await client.transactions.gas_price()).Wei * 1.5)
...
class Transactions:
    ...
    async def current_gas_price(self) -> Wei:
        print("This method will be deprecated in a future update. Use 'gas_price' instead.")
        return await self.client.transactions.gas_price()
    ...
    async def gas_price(self) -> Wei:
        ...
        """
        return Wei(await self.client.w3.eth.gas_price)
    ...
    async def find_txs(
        ...
        Args:
            ...
            coin_txs (list[dict] | None): coin txs
